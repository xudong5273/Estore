...
insert into orderstatus values(2,'processiong','in processing');
insert into orderstatus values(3,'finished','order processed');

create table payway(
	paywayid number primary key,
...
create table orders(
	...
	cost  number,
	cardno varchar2(16),
	userid varchar2(16) references s_user(userid),
	statusid number references orderstatus(statusid),
	paywayid number references payway(paywayid)
);
...
insert into product values(1,
					'JAVA编程思想：第3版',
					'从本书获得的各项大奖以及来自世界各地的读者评论中，不难看出这是一本经典之作。本书作者有多年的教
					...
					95,1,'（美）Bruce Eckel','机械工业出版社',796,'images/product/zcover.gif'
					);
...
　　						本书结合大量典型的实例，详细介绍了运用目前最成熟的Hibernate2.1版本进行Java对象持久化的技术。
					...
					种模型进行映射的技术，还能获得设计与开发Java对象模型和关系数据模型的先进经验。',
					59,1,'孙卫琴','电子工业出版社',600,'images/product/zcover2.gif'
					);
insert into product values(3,
					'Java 2核心技术（第6版） 卷I：基础知识',
					' 如果你已经是Java程序员，想了解Java的最新特性，该怎么办呢?去问一问身边的Java专家，他们会告诉
...
　　				本书为Java平台开发者提供了以下新信息：<br>
		...
		　　●动态代理类、内部类、Java平台事件模型、流、文件管理以及其他内容<br>
			...
			了著名的计算机技术图书出版社 Apress。',
					75,1,'(美)Gary Cornell','机械工业出版社',698,'images/product/zcover3.gif'
					);
insert into product values(4,
					'Effective Java中文版',
					'书介绍了在Java编程中57条极具实用价值的经验规则，这些经验规则涵盖了大多数开发人员每天所面
		...
		Specificationy》（Second Edition）的作者之一 ',
					39,1,'(美)Joshua Bloch','机械工业出版社',214,'images/product/zcover4.gif'

					);
insert into product values(5,
					'Java与模式',
					'本书是一本讲解设计原则以及最为常见的设计模式的实用教材，目的是为工作繁忙的Java系统设计师提供一
			...
			自学教材和使用手册',
					88,1,'阎宏','电子工业出版社',1024,'images/product/zcover5.gif'
					);
insert into product values(6,
					'深入浅出Hibernate',
					'本书由互联网上影响广泛的开放文档OpenDoc系列自由文献首份文档“Hibernate开发指南”发展而来。在编
			...
			给初学者提供了一个快带起步的基础。本书适合于Hibernate的各个阶层的读者。',
					59,1,'夏昕 曹晓钢 唐勇','电子工业出版社',512,'images/product/zcover6.gif'
					);
insert into product values(7,
					'Tomcat与Java Web开发技术详解',
					'本书详细介绍了在最新Tomcat 5版本上开发Java Web应用的各种技术。主要内容包括：Tomcat和Java 
		...
		Tomcat技术和开发Java Web应用的必备的实用手册。',
					45,1,'孙卫琴 李洪成','电子工业出版社',438,'images/product/zcover7.gif'
					);
insert into product values(8,
					'精通Spring',
					'本书深入剖析了当前流行的轻量级开发框架Spring技术。总共分成3部分。第一部分，重点阐述Spring的
			...
			、J2EE开发者阅读。',
					39,1,'罗时飞','电子工业出版社',425,'images/product/zcover8.gif'
					);
